...
# ==========================================
    ...
    if mode == "💬 Chat with Hero":
        ...
        # UPDATED: Load all the new columns
                ...
                with c2:
                    with st.spinner(f"{selected_alias} is thinking..."):
                        try:
                            ...
                            if st.session_state.valid_models:
                                flash_options = [m for m in st.session_state.valid_models if "flash" in m]
                                if flash_options:
    ...
    # 1. CHARACTER VAULT
        ...
        with col_roster:
            cols_needed = ["Name", "Alias", "Universe", "Role", "Age", "Origin", "Personality", "Strength", "Magic", "Power", "Weakness", "Costume", "Image_Path"]
            df = load_data(CHAR_FILE, cols_needed)
                        ...
                        if i + j < len(df):
                            ...
                            with cols[j]:
                                with st.container(border=True):
                                    ...
                                    with st.expander("👕 Costume"):
                                        st.write(row['Costume'])
            else:
